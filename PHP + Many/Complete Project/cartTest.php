...
// Handle Payment (PayPal or COD)
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    // Handle Profile Update
    if (isset($_POST['checkout']) || isset($_POST['update_profile']) || isset($_POST['payment_method'])) {
        // Directly get the input values for profile update
        $phone = $_POST['phone'] ?? '';
        $address = $_POST['address'] ?? '';
        $user_id = (int) ($_POST['user_id'] ?? 0);

        // Check if phone or address have changed
        if ($phone !== $profile['phone_number'] || $address !== $profile['address']) {
            // Direct SQL query (without any sanitization)
            $sql = "UPDATE tbl_profile SET phone_number = '$phone', address = '$address' WHERE user_id = $user_id";

            if ($conn->query($sql)) {
                echo "<p>Your profile has been updated successfully.</p>";
            } else {
                echo "<p>Error updating your profile: " . $conn->error . "</p>";
            }
        } else {
            echo "<p>No changes detected in your profile.</p>";
        }
    }

    // Handle Payment (COD or PayPal)
    if (isset($_POST['payment_method'])) {
        $payment_method = $_POST['payment_method'];

        // For Cash on Delivery (COD)
        if ($payment_method == 'cod') {
            // Update the cart status and payment mode for COD
            $stmt = $conn->prepare("UPDATE tbl_cart SET status = 3, payment_mode = 'COD' WHERE cart_id = ?");
            $stmt->bind_param("i", $cart_id);
            $stmt->execute();
            $stmt->close();

            // Redirect to order confirmation page after processing COD
        ...
        if ($payment_method == 'paypal') {
            // After PayPal payment is completed, process the payment
            $order_id = $_POST['order_id'];
            $payment_status = $_POST['payment_status'];

            if ($payment_status == 'completed') {
                // Transaction details to insert into the database
                $transaction_details = json_encode([
                    'order_id' => $order_id,
                    'status' => $payment_status,
                    'payer_email' => $_POST['payer_email'], // Add this if you have it from PayPal response
                    'payment_date' => date('Y-m-d H:i:s')
                ]);

                // Update the cart status and transaction details for PayPal
                ...
                if ($stmt->execute()) {
                    echo json_encode(['status' => 'success', 'message' => 'Payment completed. Thank you for your purchase!']);
                } else {
                    echo json_encode(['status' => 'error', 'message' => 'There was an error processing your PayPal payment.']);
                }
            }
            exit;
        }
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            $('input[name="payment_method"]').change(function() {
                if ($(this).val() === 'paypal') {
                    $('#paypal-form').show(); // Show PayPal form
                } else {
                    $('#paypal-form').hide(); // Hide PayPal form
                }
            });
                    ...
                    return actions.order.capture().then(function(details) {
                        ...
                        $.ajax({
                            ...
                            data: {
                                ...
                                order_id: data.orderID, // Send order ID
            ...
            <div id="checkout-form">
                ...
                <form method="POST" action="">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" id="user_id" name="user_id" class="form-control" value="<?php echo htmlspecialchars($profile['user_id']); ?>" readonly />
                    </div>
                    <div class="form-group">
                        <label for="phone">Phone Number</label>
                        <input type="text" id="phone" name="phone" class="form-control" value="<?php echo htmlspecialchars($profile['phone_number']); ?>" />
                    </div>
                    <div class="form-group">
                        <label for="address">Address</label>
                        <textarea id="address" name="address" class="form-control"><?php echo htmlspecialchars($profile['address']); ?></textarea>
                    </div>
                    ...
                    <div id="paypal-form" style="display: none;">
                        <!-- PayPal button will be dynamically injected here when PayPal is selected -->
                        <div class="form-group">
                            <label>PayPal Payment</label>
                            <div id="paypal-button-container"></div>
                        </div>
                    </div>
                    <!-- Add name="checkout" to the submit button -->
                    <button type="submit" name="checkout" class="btn btn-success">Complete Checkout</button>
                </form>
            </div>
        <?php else: ?>
