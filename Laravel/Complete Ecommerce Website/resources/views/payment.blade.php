...
<body>
    ...
    <div class="container mt-5">
        ...
        @if(number_format($totalAmount, 2) == '0.00')
            <script>
                window.location.href = '/';
            </script>
        @else
            <p>Congratulations, <strong>{{ ucwords(Auth::user()->name) }}</strong>! Your details were successfully updated, and now you're on the payment page.</p>

            <p><strong>Total Amount: ${{ number_format($totalAmount, 2) }}</strong></p> <!-- Display total amount -->
                    ...
                    paypal.Buttons({
                        createOrder: function(data, actions) {
                            return actions.order.create({
                                purchase_units: [{
                                    amount: {
                                        value: '{{ $totalAmount }}' // Dynamically use the total amount
                                    }
                                }]
                            });
                        },
