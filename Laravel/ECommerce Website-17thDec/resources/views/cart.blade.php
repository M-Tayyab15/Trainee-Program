...
function updateUserProfile(userDetails) {
    $.ajax({
        url: '{{ route('update.user.profile') }}', // Route to update user profile (phone and address)
        method: 'POST',
        data: {
            _token: '{{ csrf_token() }}',
        ...
        success: function(response) {
            if (response.status === 'success') {
                ...
                window.location.href = '{{ route('payment.page') }}'; // Redirect to the payment page
            } else {
...
<div class="container mt-5 mb-5" id="cart-container">
    <h1>Your Cart</h1>
    <input type="hidden" id="cart-status" value="{{ $cart->status ?? 0 }}">
        ...
        <div class="list-group">
            ...
            <div class="cart-item">
                <div>
                    ...
                    @endphp
                    <img src="{{ url($image->folder . '/' . $image->filename) }}" alt="Product Image">
                </div>
    ...
    <div class="product-card">
        ...
        <div class="quantity-container">
            <button type="button" class="btn btn-secondary quantity-decrease" data-cart-id="{{ $item->cart_product_id }}">-</button>
            <span class="quantity-value">{{ $item->quantity }}</span>
            <button type="button" class="btn btn-secondary quantity-increase" data-cart-id="{{ $item->cart_product_id }}">+</button>
            <div class="total-price product-total">${{ number_format($item->product->price * $item->quantity, 2) }}</div>
        </div>
    </div>
    <button type="button" class="btn btn-danger remove-from-cart" data-cart-id="{{ $item->cart_product_id }}">
        <span>X</span> 
    </button>
    <div class="loader" ></div> 
        ...
        <div class="total-price-container">
            <h3>Total: <span id="total-price">${{ number_format($totalPrice, 2) }}</span></h3>
        </div>
...
<div id="user-details-form" class="container mb-5">
    ...
    <form method="POST"  action="{{ route('update.user.profile') }}">
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="phone" name="phone" value="{{ Auth::user()->profile->phone ?? '' }}" required>
        </div>
        <div class="mb-3">
    <label for="address" class="form-label">Address</label>
    <td><textarea name="address" class="form-control">{{ old('address', Auth::user()->profile->address ?? '') }}</textarea></td>
</div>
